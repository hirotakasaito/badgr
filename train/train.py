...
from policy import PolNetBADGR, PolNetBADGRWithLinearCost
...
def train(model, device, dataloader, optimizer, col_loss_fn, pose_loss_fn, bumpy_loss_fn, epoch, tb_writer):
    ...
    with tqdm(dataloader, ncols=100) as pbar_train:
        for batch_i, (image, action, pose, is_collision, is_bumpy) in enumerate(pbar_train):
            t1 = time.time()
            #if(batch_i>0):
                #print("#########################################")
                #print(t1-t2)
            ...
            is_bumpy = is_bumpy.to(device, dtype=torch.float32)
            optimizer.zero_grad()

            with torch.set_grad_enabled(True):
                est_collision, est_pose, est_bumpy = model(image, action)
                ...
                mean_bumpy_loss += bumpy_loss.item()
    ...
    return mean_loss/step, mean_pose_loss/step, mean_col_loss/step, mean_bumpy_loss/step, pos_img_np
...
def test_model(model, device, col_loss_fn, pose_loss_fn, bumpy_loss_fn, testloader):
    ...
    with torch.no_grad():
        ...
        val_bumpy_loss = 0
        count = 0
        for batch_i, (image, action, pose, is_collision, is_bumpy) in enumerate(testloader):
            ...
            val_bumpy_loss += bumpy_loss.item()
            count += 1
    ...
    return val_loss/count, val_pose_loss/count , val_col_loss/count, val_bumpy_loss/count, pos_img_np
...
def main():
    ...
    parser.add_argument('--input_size', default=256, type=int, help='size of input image')
    ...
    if(args.linear_cost):
        ...
        model = PolNetBADGR(args.batch_size, args.encoder_name, args.freeze, image_shape=(3, args.input_size, args.input_size), action_shape=2).to(device)

    bumpy_loss_fn = torch.nn.BCEWithLogitsLoss()
    ...
    for epoch in range(args.epoch):
        ...
        loss, pose_loss, col_loss, bumpy_loss, pos_img_np = train(model, device, train_loader, optimizer, col_loss_fn, pose_loss_fn, bumpy_loss_fn, epoch, tb_writer)
        ...
        tb_writer.add_scalar('train/bumpy_loss', bumpy_loss, epoch+1)
        ...
        if((epoch+1)%args.test_interval==0):
            ...
            test_loss, test_pose_loss, test_col_loss, test_bumpy_loss, pos_img_np = test_model(model, device, col_loss_fn, pose_loss_fn, bumpy_loss_fn, test_loader)
            ...
            tb_writer.add_scalar('test/bumpy_loss', test_bumpy_loss, epoch+1)
